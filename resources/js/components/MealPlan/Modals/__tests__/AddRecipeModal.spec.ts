...
vi.stubGlobal(
    'route',
    vi.fn((name: string) => `mocked-route/${name}`),
);
...
describe('AddRecipeModal', () => {
    ...
    it('adds a recipe to the meal plan when addRecipe is called', async () => {
        ...
        expect(axios.post).toHaveBeenCalledWith('mocked-route/meal-plans.add-recipe', {
            meal_plan_id: 1,
            recipe_id: 123,
    ...
    it('handles API errors gracefully when adding a recipe', async () => {
        ...
        expect(console.error).toHaveBeenCalledWith('Error adding recipe to meal plan:', expect.any(Error));
