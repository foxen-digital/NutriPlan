...
describe('CopyPlanModal', () => {
  ...
  const stubs = {
    ...
    Input: {
      template: '<input :id="id" :value="modelValue" @input="$emit(\'update:modelValue\', $event.target.value)" :type="type" :min="min" :max="max" :step="step" :class="className" :placeholder="placeholder" :required="required" />',
