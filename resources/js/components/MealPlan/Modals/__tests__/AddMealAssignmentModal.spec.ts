...
describe('AddMealAssignmentModal', () => {
  ...
  const stubs = {
    ...
    Input: {
      template: '<input :id="id" :value="modelValue" @input="$emit(\'update:modelValue\', $event.target.value)" :type="type" :min="min" :max="max" :step="step" :class="className" />',
      props: ['id', 'modelValue', 'type', 'min', 'max', 'step', 'className'],
      emits: ['update:modelValue']
    },
    Select: {
      template: '<select :id="id" :value="modelValue" @change="$emit(\'update:modelValue\', $event.target.value)" :class="className"><option v-for="option in options" :key="option.value" :value="option.value">{{ option.label }}</option></select>',
